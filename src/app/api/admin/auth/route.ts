...
export async function POST(req: NextRequest) {
  ...
  if (!secret) {
    return NextResponse.json(
      { ok: false, error: "ADMIN_SECRET is not configured on this server." },
      { status: 503 }
    );
  }
  ...
  if (password !== secret) {
    ...
    loginUrl.searchParams.set("next", next.startsWith("/") ? next : "/audit");
    loginUrl.searchParams.set("error", "1");
    return NextResponse.redirect(loginUrl, { status: 303 });
  }

  // Correct password — set secure HttpOnly cookie and redirect
